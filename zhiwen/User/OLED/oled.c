...
//////////////字体取反
void OLED_ShowStringInvert(u8 x,u8 y,const u8 *p,u8 size)
{	
    while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
    {       
        if(x>(128-(size/2))){x=0;y+=size;}
...
{ 	
 ...
 #else
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9|GPIO_Pin_7|GPIO_Pin_8;				 //PC0,1 OUT推挽输出
 	GPIO_Init(GPIOB, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOB,GPIO_Pin_9|GPIO_Pin_7|GPIO_Pin_8);						 //PC0,1 OUT  输出高

//	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;				 //PG15 OUT推挽输出	  RST
// 	GPIO_Init(GPIOB, &GPIO_InitStructure);
// 	GPIO_SetBits(GPIOB,GPIO_Pin_15);						 //PG15 OUT  输出高
