...
//GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(5));	//IO口输出低电平，亮
//GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(5));	//IO口输出高电平，灭
//GPIOA_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(4));	//IO口输出低电平，亮
//GPIOA_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(4));	//IO口输出高电平，灭
...
//      __asm("nop");    
  while(i--)
  {
    if(data&0x80)
     {
//       GPIOD_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(3));;;;
			 LCD_SDIN=1;
      }
    else{
//		GPIOD_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(3));;;;
...
//	GPIOD_PDDR|=GPIO_PDDR_PDD(GPIO_PIN(1)|GPIO_PIN(3)|GPIO_PIN(5)|GPIO_PIN(4));
//	
//  
//	GPIOD_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(1));
//	//LCD_CS=1;	//预制SLK和SS为高电平   	
//	
//	GPIOD_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(5));
//	LCD_DLY_ms(50);
//	GPIOD_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(5));
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_10|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_7;	 //PD3,PD6推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50MHz
	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //初始化GPIOD3,6
 	GPIO_ResetBits(GPIOB,GPIO_Pin_10);	//PD3,PD6 输出高
	LCD_SCLK=1;
	LCD_RST=0;
	LCD_DLY_ms(50);
	LCD_RST=1;
...
// 	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //初始化GPIOD3,6
// 	GPIO_SetBits(GPIOB,GPIO_Pin_9);	//PD3,PD6 输出高
//	
////	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //初始化GPIOD3,6
// 	GPIO_ResetBits(GPIOB,GPIO_Pin_7);	//PD3,PD6 输出高
//	
//	LCD_DLY_ms(50);
//	
////	GPIO_Init(GPIOB, &GPIO_InitStructure);	  //初始化GPIOD3,6
// 	GPIO_SetBits(GPIOB,GPIO_Pin_7);	//PD3,PD6 输出高
...
//==============================================================
//函数名： void Image_set(byte x,byte y)
//功能描述：设置一个点（x,y）
//参数：真实坐标值(x,y),x的范围0～127，y的范围0～64
//返回：无
//==============================================================
void Image_set(byte x,byte y)
{
	...
	byte data2;
	data2=y/8;
	my_image[x+data2*127]|=0x01<<(y%8); 	
}
//==============================================================
//函数名： Draw_BinarySet(byte x,byte y,u8 video[][])
//功能描述：设置一个点（x,y）
//参数：真实坐标值(x,y),x的范围0～127，y的范围0～64
//返回：无
//==============================================================
void Draw_BinarySet(byte x,byte y,u8* video,u8 binary)
{
	
}
//==============================================================
//函数名： void Image_clear()
//功能描述：清除图像
//参数：真实坐标值(x,y),x的范围0～127，y的范围0～64
//返回：无
//==============================================================
...
void LCD_P8x16StrInvert(byte x,byte y,byte ch[])
{
  ...
  {    
    ...
    LCD_Set_Pos(x,y);    
  	for(i=0;i<8;i++)     
  	  LCD_WrDat(F8X16[c*16+i]^0xFF);
  	LCD_Set_Pos(x,y+1);    
  	for(i=0;i<8;i++)     
  	  LCD_WrDat(F8X16[c*16+i+8]^0xFF);  
  	x+=8;
  	j++;
  }
}
...
void LCD_PrintInvert(byte x, byte y, byte ch[])
{
	...
	while(ch[ii] != '\0')
	{
		...
		else
		{
			ch2[0] = ch[ii];	
			ch2[1] = '\0';			//字母占一个字节
			LCD_P8x16StrInvert(x , y , ch2);	//显示字母
			x += 8;
			ii+= 1;
		}
	}
...
void LCD_P6x8Int(u8 x,u8 y,u8 ch[],int num)
{
  ...
  sprintf(data,"%3d ",num);//3
  LCD_P6x8Str(x+24,y,(u8 *)data);  //汉字字符串显示
}
